...
const PDF_URL = `${import.meta.env.BASE_URL}APMicro-Exam1.pdf`;
const NUM_QUESTIONS = 60;
const CHOICES = ['A', 'B', 'C', 'D', 'E'];

// Princeton AP Microeconomics MCQ Answer Key
...
const APMicroPracticeExamMCQPrinceton: React.FC = () => {
  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-slate-100 relative">
      <button
        onClick={() => navigate(-1)}
        className="absolute top-4 left-4 px-4 py-2 bg-white text-fuchsia-600 rounded-lg shadow hover:bg-slate-50 transition-colors flex items-center gap-2 z-10"
      >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
            <h1 className="text-4xl font-extrabold text-fuchsia-800 tracking-tight">AP Microeconomics Princeton MCQ Practice Exam</h1>
            <p className="mt-2 text-lg text-slate-600">60 Questions | 70 Minutes</p>
        </div>
        <Stopwatch />
        <div className="mt-8 w-full flex flex-col md:flex-row gap-8 justify-start items-start">
          ...
          <div className="flex-[3] min-w-[450px] max-w-5xl bg-white shadow-xl rounded-2xl flex flex-col items-center border border-slate-200">
            <div className="w-full p-4 bg-slate-50 rounded-t-2xl border-b border-slate-200">
                <h2 className="text-xl font-bold text-center text-fuchsia-700">Princeton AP Microeconomics Practice Exam Document</h2>
            </div>
            <iframe
              src={PDF_URL}
              width="100%"
              height="900px"
              title="AP Microeconomics Princeton Practice Exam PDF"
          ...
          <div className="flex-1 min-w-[400px] max-w-md bg-white shadow-xl p-6 rounded-2xl border border-slate-200 self-stretch">
            <div className="p-4 bg-slate-50 rounded-t-xl border-b border-slate-200 -m-6 mb-6">
                <h2 className="text-2xl font-bold mb-1 text-center text-fuchsia-800">Digital Scantron</h2>
            </div>
            {!submitted && (
              <div className="text-center">
                <button
                  className="mb-4 w-full px-6 py-3 bg-fuchsia-600 text-white text-lg rounded-lg font-semibold shadow-md hover:bg-fuchsia-700 transition-transform transform hover:scale-105"
            ...
            {submitted && (
              <div className="mb-6 p-4 rounded-lg bg-fuchsia-50 border border-fuchsia-200 text-center">
                <div className="text-2xl font-bold text-fuchsia-800">
                  Your Score: {score} / {NUM_QUESTIONS}
                </div>
                <div className="text-xl font-semibold text-fuchsia-700 mt-1">
...
export default APMicroPracticeExamMCQPrinceton;
