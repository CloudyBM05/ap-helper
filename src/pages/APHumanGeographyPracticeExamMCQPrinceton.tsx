...
const PDF_URL = `${import.meta.env.BASE_URL}APHUG-Exam1.pdf`;
...
const Stopwatch: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center mb-6 bg-white p-4 rounded-xl shadow-md border border-slate-200">
      <div className="text-4xl font-bold font-mono mb-3 text-emerald-700">{formatTime(seconds)}</div>
      <div className="flex gap-3">
        <button
          className="px-5 py-2 rounded-lg bg-emerald-500 text-white font-semibold shadow-sm hover:bg-emerald-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <button
          className="px-5 py-2 rounded-lg bg-slate-400 text-white font-semibold shadow-sm hover:bg-slate-500 transition-colors"
          onClick={handleReset}
        >
          Reset
        </button>
...
const APHumanGeographyPracticeExamMCQPrinceton: React.FC = () => {
  const navigate = useNavigate();
  const [answers, setAnswers] = useState<(string | null)[]>(Array(NUM_QUESTIONS).fill(null));
  const [crossed, setCrossed] = useState<boolean[][]>(Array.from({ length: NUM_QUESTIONS }, () => Array(CHOICES.length).fill(false)));
  ...
  const getButtonClass = (qIdx: number, c: string, cIdx: number) => {
    ...
    if (answers[qIdx] === c) {
      return `${baseClass} bg-emerald-500 border-emerald-700 text-white font-bold ring-2 ring-emerald-300`;
    }
  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-slate-100 relative">
      <button
        onClick={() => navigate(-1)}
        className="absolute top-4 left-4 px-4 py-2 bg-white text-emerald-600 rounded-lg shadow hover:bg-slate-50 transition-colors flex items-center gap-2 z-10"
      >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
            <h1 className="text-4xl font-extrabold text-emerald-800 tracking-tight">Princeton Book AP Human Geography MCQ</h1>
            <p className="mt-2 text-lg text-slate-600">75 Questions | 60 Minutes</p>
        </div>
        <Stopwatch />
        <div className="mt-8 w-full flex flex-col md:flex-row gap-8 justify-start items-start">
          ...
          <div className="flex-[3] min-w-[450px] max-w-5xl bg-white shadow-xl rounded-2xl flex flex-col items-center border border-slate-200">
            <div className="w-full p-4 bg-slate-50 rounded-t-2xl border-b border-slate-200">
                <h2 className="text-xl font-bold text-center text-emerald-700">Princeton Book AP Human Geography Exam PDF</h2>
            </div>
            <iframe
              src={PDF_URL}
              width="100%"
              height="900px"
              title="Princeton Book AP Human Geography MCQ PDF"
          ...
          <div className="flex-1 min-w-[400px] max-w-md bg-white shadow-xl p-6 rounded-2xl border border-slate-200 self-stretch">
            ...
            {!submitted && (
              <div className="text-center">
                <button
                  className="mb-4 w-full px-6 py-3 bg-emerald-600 text-white text-lg rounded-lg font-semibold shadow-md hover:bg-emerald-700 transition-transform transform hover:scale-105"
...
export default APHumanGeographyPracticeExamMCQPrinceton;
