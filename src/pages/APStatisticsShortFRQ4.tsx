...
const PDF = '/APSTAT-ShortFRQ4.pdf';

const aiPrompt = `STRICT AI PROMPT (TOKEN-EFFICIENT, GRADER-FRIENDLY):\nYou are a strict AP Statistics grader. Evaluate each part of the 2025 AP Statistics FRQ below.\nFormat — BE STRICT:\n\nLabel each subpart (A(i), A(ii), B(i), B(ii))\n\nOutput score as "1/1" or "0/1"\n\nGive brief, justification-based explanation only if needed (max 2 lines)\n\nUse minimal words. No restating questions. No intros. No grading logic.\n\nAssume student work is visible and shows calculations where needed.\n\nQuestion Content (Summary for Model):\nIn Country W, 22% of students use a homework-help app weekly. Karen thinks her school (2,000+ students) has a higher proportion. She takes an SRS of 130 students; 38 report using the app weekly.\nTask: Conduct inference test at α = 0.05 to test if her school's proportion is greater than 0.22. Justify.\n\nSubparts:\nA(i). State null and alternative hypotheses.\nA(ii). Check all conditions (random, 10%, normal).\nB(i). Calculate test statistic and p-value.\nB(ii). Make conclusion in context. Justify based on p-value and α.`;

const APStatisticsShortFRQ4: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('apstat-frq-answers-apstatisticsshortfrq4');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (Object.keys(answers).length > 0) {
      localStorage.setItem('apstat-frq-answers-apstatisticsshortfrq4', JSON.stringify(answers));
    }
  }, [answers]);
...
const handleChange = (part: string, value: string) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      localStorage.removeItem('apstat-frq-answers-apstatisticsshortfrq4');
    } catch (err: any) {
        ...
        <div className="flex flex-col md:flex-row justify-center items-start gap-8">
          ...
          <div className="flex-[1.5] min-w-[400px] max-w-5xl bg-white shadow-lg p-6 flex flex-col rounded-2xl border border-purple-100">
            <h2 className="text-xl font-bold mb-4 text-center text-purple-700">
              2025 Collegeboard Short FRQ Set 1 - Question 4
            </h2>
            <iframe
              src={PDF}
              title="AP Statistics Short FRQ #4 PDF"
...
export default APStatisticsShortFRQ4;
