...
const FRQ_CONTENT = {
  title: 'Collegeboard 2025 Concept Application Set 2',
...
const CRITERIA = [
  'A. Names and describes a valid voluntary migration type (e.g., transnational, step, chain, rural–urban).',
  'B. Shows how migration changes city economy (e.g., labor supply, tax base, services).',
  'C. Names and describes urban housing discrimination (e.g., redlining, blockbusting, exclusionary zoning).',
  'D. Explains infrastructure impact on society (e.g., transport, water, waste, equity).',
  'E. States urban sustainability challenge (e.g., air pollution, sprawl, waste, energy).',
  'F. Shows how urban design (e.g., walkability, mixed-use, infill) improves city livability.',
  'G. Gives valid criticism of urban design (e.g., gentrification, cost, displacement, greenwashing).',
];

const APHumanGeographyConceptApplicationSet2 = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('aphug-concept-application-set2');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (Object.keys(answers).length > 0) {
      localStorage.setItem('aphug-concept-application-set2', JSON.stringify(answers));
    }
  }, [answers]);
...
export default APHumanGeographyConceptApplicationSet2;
