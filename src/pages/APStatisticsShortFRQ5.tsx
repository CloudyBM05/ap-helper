...
const APStatisticsShortFRQ5: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('apstat-frq-answers-apstatisticsshortfrq5');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (Object.keys(answers).length > 0) {
      localStorage.setItem('apstat-frq-answers-apstatisticsshortfrq5', JSON.stringify(answers));
    }
  }, [answers]);
...
const handleChange = (part: string, value: string) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      localStorage.removeItem('apstat-frq-answers-apstatisticsshortfrq5');
    } catch (err: any) {
