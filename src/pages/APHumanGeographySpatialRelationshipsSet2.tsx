...
const FRQ_CONTENT = {
  title: 'Collegeboard 2025 Spatial Relationships Set 2',
...
const CRITERIA = [
  'A. Identifies largest food export category correctly using chart data (e.g., cereals, dairy, meat, etc.).',
  'B. Describes how meat, dairy, seafood exports differ between the two countries using chart.',
  'C. Defines comparative advantage (ability to produce at lower opportunity cost).',
  'D. Describes Green Revolution effect (e.g., increased yields, food security in developing countries).',
  'E. Explains how ag tech (e.g., GMOs, irrigation, mechanization) increases carrying capacity.',
  'F. States degree (low/moderate/high) and explains interdependence (e.g., trade reliance, food supply links).',
  'G. Explains chart limitations (e.g., no GDP/income data, no economic sector info).',
];

const APHumanGeographySpatialRelationshipsSet2 = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('aphug-spatial-relationships-set2');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (Object.keys(answers).length > 0) {
      localStorage.setItem('aphug-spatial-relationships-set2', JSON.stringify(answers));
    }
  }, [answers]);
...
export default APHumanGeographySpatialRelationshipsSet2;
