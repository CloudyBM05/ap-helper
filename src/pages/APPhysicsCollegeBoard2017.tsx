...
const PDF_URL = `${import.meta.env.BASE_URL}APPhysics-Exam.pdf`;
const NUM_QUESTIONS = 50;
const CHOICES = ['A', 'B', 'C', 'D'];

// AP Physics CollegeBoard 2017 MCQ Answer Key
const CORRECT_ANSWERS = [
  'D','A','D','B','B','C','C','C','A','D',
  'B','C','C','D','C','A','A','B','A','D',
  'C','C','B','D','B','B','D','D','C','D',
  'B','C','D','D','B','B','C','C','A','A',
  'C','D','C','B','A','B,D','B,C','C,D','A,B','A,B'
];
...
const APPhysicsCollegeBoard2017: React.FC = () => {
  ...
  const handleSubmit = () => {
    ...
    for (let i = 0; i < NUM_QUESTIONS; i++) {
      if (answers[i]) {
        ...
        const userAnswer = answers[i] as string;
        
        // For questions 46-50 with multiple acceptable answers
  ...
  const getButtonClass = (qIdx: number, c: string, cIdx: number) => {
    ...
    if (submitted) {
      const correctAnswer = CORRECT_ANSWERS[qIdx];
      ...
      if (correctAnswer && correctAnswer.includes(',')) {
        ...
        const acceptableAnswers = correctAnswer.split(',');
        isCorrectAnswer = acceptableAnswers.includes(c);
      } else {
        // For single answer questions
  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-slate-100 relative">
      <button
        onClick={() => navigate(-1)}
        className="absolute top-4 left-4 px-4 py-2 bg-white text-teal-600 rounded-lg shadow hover:bg-slate-50 transition-colors flex items-center gap-2 z-10"
      >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
            <h1 className="text-4xl font-extrabold text-slate-800 tracking-tight">AP Physics CollegeBoard 2017 MCQ Practice Exam</h1>
            <p className="mt-2 text-lg text-slate-600">50 Questions | 90 Minutes</p>
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg inline-block">
              <p className="text-yellow-800 text-sm font-medium">
                ⚠️ This exam is from 2017 and may not reflect current AP Physics standards
              </p>
            </div>
        </div>
        <Stopwatch />
        <div className="mt-8 w-full flex flex-col md:flex-row gap-8 justify-start items-start">
          ...
          <div className="flex-[3] min-w-[450px] max-w-5xl bg-white shadow-xl rounded-2xl flex flex-col items-center border border-slate-200">
            <div className="w-full p-4 bg-slate-50 rounded-t-2xl border-b border-slate-200">
                <h2 className="text-xl font-bold text-center text-slate-700">AP Physics CollegeBoard 2017 Practice Exam Document</h2>
            </div>
            <iframe
              src={PDF_URL}
              width="100%"
              height="900px"
              title="AP Physics CollegeBoard 2017 Practice Exam PDF"
          ...
          <div className="flex-1 min-w-[400px] max-w-md bg-white shadow-xl p-6 rounded-2xl border border-slate-200 self-stretch">
            ...
            {!submitted && (
              <div className="text-center">
                <button
                  className="mb-4 w-full px-6 py-3 bg-teal-600 text-white text-lg rounded-lg font-semibold shadow-md hover:bg-teal-700 transition-transform transform hover:scale-105"
            ...
            {submitted && (
              <div className="mb-6 p-4 rounded-lg bg-teal-50 border border-teal-200 text-center">
                ...
                <div className="text-xl font-semibold text-teal-700 mt-1">
            ...
            <div className="text-sm text-slate-500 mb-4 text-center w-full">
              ...
              <br />
              <span className="text-xs mt-1 block">Note: Questions 46-50 have multiple acceptable answers.</span>
            </div>
...
export default APPhysicsCollegeBoard2017;
