...
const PDF_URL = `${import.meta.env.BASE_URL}APCSP-Exam1.pdf`;
const NUM_QUESTIONS = 70;
const CHOICES = ['A', 'B', 'C', 'D'];

// AP Computer Science Principles Practice Book Exam Answer Key
...
// Multi-select questions mapping (questions 131-138 from answer key map to questions 63-70)
const MULTI_SELECT_ANSWERS: Record<number, string[]> = {
  63: ['A','D'], // Question 131 -> Boolean Expressions
  64: ['B','C'], // Question 132 -> Identifying and Correcting Errors
  65: ['B','C'], // Question 133 -> Conditionals
  66: ['A','D'], // Question 134 -> Conditionals
  67: ['A','D'], // Question 135 -> Random Values
  68: ['A','C'], // Question 136 -> Identifying and Correcting Errors
  69: ['A','D'], // Question 137 -> Extracting Information from Data
  70: ['A','C'], // Question 138 -> Calling Procedures
};
...
const APCSPPracticeExamMCQPracticeBook: React.FC = () => {
  ...
  const handleSelect = (qIdx: number, choice: string) => {
    const isMultiSelect = qIdx >= 62; // Questions 63-70 are multi-select
  ...
  const handleSubmit = () => {
    let correct = 0;
    for (let i = 0; i < 62; i++) { // Single-select questions (1-62)
      if (answers[i] && answers[i] === CORRECT_ANSWERS[i]) correct++;
    }
    
    // Multi-select questions (63-70)
    for (let i = 62; i < NUM_QUESTIONS; i++) {
    ...
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-slate-100 relative">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
            <h1 className="text-4xl font-extrabold text-violet-800 tracking-tight">AP Computer Science Principles Practice Book MCQ Exam</h1>
            <p className="mt-2 text-lg text-slate-600">70 Questions | 120 Minutes | Questions 63-70 are multi-select</p>
        </div>
        <Stopwatch />
        <div className="mt-8 w-full flex flex-col md:flex-row gap-8 justify-start items-start">
          ...
          <div className="flex-[3] min-w-[450px] max-w-5xl bg-white shadow-xl rounded-2xl flex flex-col items-center border border-slate-200">
            <div className="w-full p-4 bg-slate-50 rounded-t-2xl border-b border-slate-200">
                <h2 className="text-xl font-bold text-center text-violet-700">AP Computer Science Principles Practice Book Exam Document</h2>
            </div>
            <iframe
              src={PDF_URL}
              width="100%"
              height="900px"
              title="AP Computer Science Principles Practice Book Exam PDF"
          ...
          <div className="flex-1 min-w-[400px] max-w-md bg-white shadow-xl p-6 rounded-2xl border border-slate-200 self-stretch">
            ...
            <div className="text-sm text-slate-500 mb-4 text-center w-full">
              <span>Left-click to select. Right-click to cross out. Questions 63-70 allow multiple selections.</span>
            </div>
                  ...
                  {Array.from({ length: NUM_QUESTIONS }).map((_, qIdx) => (
                    <tr key={qIdx} className="hover:bg-slate-50">
                      <td className="font-bold p-2 text-slate-700">
                        {qIdx + 1}
                        {qIdx >= 62 && <span className="text-xs text-violet-600 ml-1">(multi)</span>}
                      </td>
                      {CHOICES.map((c, cIdx) => (
...
export default APCSPPracticeExamMCQPracticeBook;
