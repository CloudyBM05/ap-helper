...
const PDF_URL = `${import.meta.env.BASE_URL}AP-Helper-APUSH.pdf`;

const NUM_QUESTIONS = 55;
const CHOICES = ['A', 'B', 'C', 'D'];

// Correct answers for AP-Helper Original MCQ
...
const APUSHPracticeExamMCQAPHelper: React.FC = () => {
  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-slate-100 relative">
      ...
      <button
        ...
        className="absolute top-4 right-4 px-4 py-2 bg-white text-blue-600 rounded-lg shadow hover:bg-slate-50 transition-colors z-10"
      >
        {showPdf ? 'Hide' : 'Show'} Exam PDF
      </button>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
            <h1 className="text-4xl font-extrabold text-slate-800 tracking-tight">AP-Helper Original Practice Exam</h1>
            <p className="mt-2 text-lg text-slate-600">55 Questions | 55 Minutes</p>
        </div>
        <Stopwatch />
        <div className={`grid ${showPdf ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'} gap-8`}>
          {showPdf && (
            <div className="pdf-container h-[calc(100vh-200px)] rounded-lg shadow-lg overflow-hidden border border-slate-200">
                <iframe src={PDF_URL} width="100%" height="100%" title="AP-Helper Original Practice Exam PDF"></iframe>
            </div>
          )}
          <div className={`${showPdf ? '' : 'max-w-4xl mx-auto w-full'}`}>
            <div className="bg-white p-6 rounded-lg shadow-lg border border-slate-200">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-200">
                    <h2 className="text-2xl font-bold text-slate-800">Digital Scantron</h2>
                ...
                {!submitted && (
                  <div className="text-center mb-4">
                    <button
                      className="w-full px-6 py-3 bg-blue-600 text-white text-lg rounded-lg font-semibold shadow-md hover:bg-blue-700 transition-transform transform hover:scale-105"
...
export default APUSHPracticeExamMCQAPHelper;
